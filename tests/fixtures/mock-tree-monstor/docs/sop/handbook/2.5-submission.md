...
> ⚠️ **歷史快照（v2.0 變動起）**：本檔為 `tests/fixtures/mock-tree-monstor/` 安裝測試快照，**內文仍為 v1.x 「三層產出物 + HTML」舊規則**，不隨 handbook 同步更新（v2.0 起規則改為「兩層產出物 + 無 HTML」，見 `docs/sop/handbook/changelog.md` v2.0 條目）。讀者請勿誤判 fixture 是 current SOP。
