...
- [x] 關鍵決策點 SWOT 已展開（[依 dav-planner §2.6，如無則填 N/A] — 只有不可逆變更/高成本/糾結/戰略影響 才展開）
- [x] 最終選項 SWOT 已寫進對應 backlog AC 欄（[依 dav-planner §4.3.1，如無則填 N/A] — 只記最終選項、純文字前綴、不用 `>` blockquote）
- [x] 用戶明確確認：「<用戶原話>」
→ 進入 §2.2 Design
```
