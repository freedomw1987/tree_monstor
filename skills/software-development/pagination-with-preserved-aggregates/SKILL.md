...
- **`limit: -1` 唔好用喺 dashboard / list render page — 用途只係 dropdown filter 同 export (2026-06-10 <project> Sprint 14)**:有人喺 `DashboardPage.tsx` 寫 `projectApi.list({ limit: -1 })` 想「載晒全部項目顯示喺 dashboard」,結果 196 個項目 card 全部 render → 截圖 91834px tall (fullPage screenshot tool 爆),PIL 都開唔到,RWD audit 完全冇 feedback。**Rule**:
